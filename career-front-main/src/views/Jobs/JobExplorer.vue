    ...
    <main class="main-content">
        ...
        >
          ...
          >
            ...
            <div class="card-main">
              ...
              <img class="card-illustration" :src="getRandomImage(index)" alt="" />
            </div>
      ...
      <aside :class="['preview-panel', { 'is-solid': hoveredJob }]">
        ...
        <div v-else class="preview-active">
          ...
          <div class="preview-tags" v-if="hoveredJob.tags?.length">
            <span
          ...
          <div class="preview-section">
            ...
            <ul class="requirements-list">
              <li v-for="(req, idx) in parseJobRequirements(hoveredJob.description)" :key="idx">{{ req }}</li>
            </ul>
            <p v-if="parseJobRequirements(hoveredJob.description).length === 0" class="desc-text">暂无详细要求，点击卡片查看完整画像</p>
          </div>
          <div class="click-tip">
...
const getRandomImage = (index) => cardImages[index % cardImages.length]

const router = useRouter()

const parseJobRequirements = (text) => {
  if (!text) return []
  ...
  for (const line of lines) {
    ...
    if (!trimmed) continue
    // 匹配"任职要求"、"岗位要求"、"职位要求"、"要求"等标题
    if (/^(?:\d+[\.\、]\s*)?(?:任职要求|岗位要求|职位要求|岗位职责|任职资格|基本要求|要求)/.test(trimmed)) {
      inSection = true
      continue
    }
    // 遇到下一个大标题则停止
    if (inSection && /^[1-9]\d*[\.\、]\s*/.test(trimmed) && !/^(?:\d+[\.\、]\s*)?(?:任职要求|岗位要求|职位要求)/.test(trimmed)) {
      break
    }
    if (inSection) {
...
.elegant-job-card {
  ...
  .card-illustration {
    ...
    opacity: 0.4;
    pointer-events: none;
    mask-image: radial-gradient(ellipse 60% 60% at center, black 20%, transparent 75%);
    -webkit-mask-image: radial-gradient(ellipse 60% 60% at center, black 20%, transparent 75%);
  }
    ...
    .requirements-list {
      ...
      li {
        ...
        &::before {
          ...
          height: 5px;
          border-radius: 50%;
          background: linear-gradient(135deg, #409eff, #8c97f6);
        }
      }
    }
